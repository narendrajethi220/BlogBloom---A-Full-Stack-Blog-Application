       ...
       <form className="flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll">
          <div className="bg-white w-full max-w-3xl p-4 md:p-10 sm:m-8 shadow rounded">
            ...
            <label htmlFor="image">
              {
              !image ?  
              <IconContext.Provider value={{className:"mt-2 text-md text-primary h-16 w-25 text-2xl p-4 rounded cursor-pointer border-1 border-dashed"}}>
                 <RiFolderUploadFill />
              </IconContext.Provider>
              :
              <img src={URL.createObjectURL(image)} alt="" className="mt-2 h-16 rounded cursor-pointer"/>
              }
              <input onChange={(e)=>setImage(e.target.files[0])} className="mt-2" type="file" id="image"  required/>
            </label>
           ...
           <div className="flex gap-2 mt-4">
            ...
            <input type="checkbox" checked={isPublisehd} className="scale-125 cursor-pointer" onChange={e=>setIsPublised(e.target.checked)}/>
           </div>
